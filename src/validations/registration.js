...
const { ENUM_ENROLLED_STATUS, ENUM_DISCONTINUATION_REASON } = require("../contants/constant");
...
const createRegistrationValidationRules = () => {
    return [
      ...
      body("classCapacity", "min length should be 2").isNumeric().isLength({ min: 2 }),
      body("waitlistCapacity", "min length should be 2").isNumeric().isLength({ min: 2 }),
      body("waitlistEnrolled", "min length should be 2").isNumeric().isLength({ min: 2 }),
